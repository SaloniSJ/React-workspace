...
function getSteps() {
  return ['ORDER PLACED', 'PREPARING', 'PARTIALLY SERVED', 'FULLY SERVED'];
}
      ...
      if (response.data.status) {
        ...
        OrderServiceAPI.fetchOrderDetailsByOrderId(payload).then(response => {
          if (response.data.status) {
            ...
            if (response.data.data.order_status === 'PREPARING') setActiveStep(1)
            if (response.data.data.order_status === 'PARTIALLY SERVED') setActiveStep(2)
            if (response.data.data.order_status === 'FULLY SERVED') setActiveStep(3)
          }
        })
      }
    })
