...
export const CustomStepper = (props) => {
  ...
  const handleNext = (order_status) => {
    ...
    const payload = { order_item_status: order_status, order_id: props.order.order_id, item_id:itemId}
    OrderServiceAPI.updateOrderItemStatus(payload).then(response => {
      if (response.data.status) {
        ...
        OrderServiceAPI.fetchOrderDetailsByOrderId(payload).then(response => {
          if (response.data.status) {
            console.log("Item Stepper==>",response)
            ...
            if (response.data.data.order_Details[0].status === 'NEW') setActiveStep(0)
            if (response.data.data.order_Details[0].status === 'PREPARING ITEM') setActiveStep(1)
            if (response.data.data.order_Details[0].status === 'ITEM READY') setActiveStep(2)
          }
        })
      }
    })
  ...
  useEffect(() => {
    ...
    OrderServiceAPI.fetchOrderDetailsByOrderId(payload).then(response => {
      if (response.data.status) {
        console.log(response)
        setItemId(response.data.data.order_Details[0].item_id)
        if (response.data.data.order_Details[0].status === 'NEW') setActiveStep(0)
        if (response.data.data.order_Details[0].status === 'PREPARING') setActiveStep(1)
        if (response.data.data.order_Details[0].status === 'ITEM READY') setActiveStep(2)
      }
    })
  }, [])
